...
边云协同（LWD prefill_only）推理把一套大模型切成两段：边侧只跑 embedding 与 unembed / lm_head，云侧跑全部 transformer 层。一次请求的完整生命周期要跨节点往返多次：prefill 阶段按 chunk 多次"边发 embedding → 云算中间层"，decode 阶段每步一次"云算完 hidden → 边做 unembed 还原 token"。
...
第 4 点是本需求的核心难点：两条数据面通道（UP / DOWN）都是严格按 seqno 顺序配对的 FIFO（HCCL P2P 无 tag），而一个请求在其生命周期内会在两条通道上占用多个 seqno。若请求中途被终止，它"尚未发生的那部分传输"就不再发生，在通道上留下 seqno 空洞；空洞不处理会卡死该通道上它之后的所有请求。
...
| 通道 | 方向 | 数据面载荷 | 一次传输粒度 |
|---|---|---|---|
| `UP` | 边 → 云 | prompt embeddings（`[N, H]` bf16） | 一个 prefill chunk（单请求批） |
...
- `UP` seqno 由边侧在"发布 `LwdRangeNotify` 成功"时原子分配（peek-then-advance），保证号只分配给真正上 wire 的块、号连续无空洞；被终止请求的未来 chunk 只是不再分配号。
...
| 通知 | 方向 | 内容 | 与终止的关系 |
|---|---|---|---|
| `LwdRequestNotify` | 边 → 云 | 请求元数据（prompt 长度 / 采样参数 / 哈希链） | 准入时预告 |
| `LwdRangeNotify` | 边 → 云 | 每个 chunk 的范围（offset / num_tokens / seqno） | chunk 预告，seqno 随之分配 |
...
| prefill 中途 | 若干 chunk 已发（edge 已 send） | 已发出的 chunk 存在**在途 send**，必须按原尺寸挂空排空（recv-and-drop） |
...
边侧"发布 `LwdRangeNotify(seqno=N)`"与"挂 `lwd_batch(seqno=N)` 派发"是同步完成的，即边侧该 chunk 的 send **必然已在途**。若此时请求被终止、且 abort 先于该陈旧预告被消费（竞态窗口），云侧 `LwdAbortNotify` 把请求移出调度器，随后 `prefill_notify_queue` 里的陈旧 `LwdRangeNotify(seqno=N)` 被丢弃、不再 post recv——于是 UP 通道在 seqno=N 处留下洞，卡死其后所有 chunk。

**必须处理**：云侧丢弃陈旧 chunk 预告时，仍须按原尺寸挂一个空 recv 排空（recv-and-drop）该在途 send，保持 UP 通道配对连续。这是 v0.23.0_lwd 采用的"排空、绝不跳过"思路，也是本需求唯一需要新增数据面处理的点。
...
4. **终止与发送的竞态必须兜底**：对端可能先于终止把数据发出（边侧"预告+派发"同步），本端已排空的 seqno 仍可能收到数据——不得崩溃，应可识别地快速失败而非静默超时。
5. **UP seqno 必须连续**：号只分配给真正上 wire 的块；被终止请求的未来 chunk 只是不再分号，而非占号后丢弃。
